...
void testDiffuse(int maxSize, int iterations) {
    ...
    for (int matrixSize = 75; matrixSize <= maxSize; matrixSize += 75) {
        ...
        Utils::log(Utils::LogLevel::DEBUG, std::cout, "Diffuse test\n", "\tMatrix size: ", matrixSize, "\n", "\tDiffuse: ", serialTimeMean, " micros\n",
                   "\tOMP Diffuse: ", ompTimeMean, " micros\n", "\tAverage team's threads number: ", threadMean, "\n", "\tSpeedup: ", speedup, "\n", "\tEfficiency: ", efficiency);
...
void testAdvect(int maxSize, int iterations) {
    ...
    for (int matrixSize = 75; matrixSize <= maxSize; matrixSize += 75) {
        ...
        speedup = (double) serialTimeMean / (double) ompTimeMean;
        efficiency = speedup / threadMean;

        Utils::log(Utils::LogLevel::DEBUG, std::cout, "Advect test\n", "\tMatrix size: ", matrixSize, "\n", "\tAdvect: ", serialTimeMean, " micros\n",
                   "\tOMP Advect: ", ompTimeMean, " micros\n", "\tAverage team's threads number: ", threadMean, "\n", "\tSpeedup: ", speedup, "\n", "\tEfficiency: ", efficiency);
...
void testProject(int maxSize, int iterations) {
    ...
    for (int matrixSize = 75; matrixSize <= maxSize; matrixSize += 75) {
        ...
        Utils::log(Utils::LogLevel::DEBUG, std::cout, "Project test\n", "\tMatrix size: ", matrixSize, "\n", "\tProject: ", serialTimeMean, " micros\n",
                   "\tOMP Project: ", ompTimeMean, " micros\n", "\tAverage team's threads number: ", threadMean, "\n", "\tSpeedup: ", speedup, "\n", "\tEfficiency: ", efficiency);
...
void advect(int N, Axis mode, std::vector<double> &value, std::vector<double> &oldValue, std::vector<double> &vX, std::vector<double> &vY, double dt) {
    ...
    for (int i = 1; i < N - 1; i++) {
        for (int j = 1; j < N - 1; j++) {
            ...
            value[index(i, j, N)] =
                    s0 * (t0 * oldValue[index(i0i, j0i, N)] + t1 * oldValue[index(i0i, j1i, N)]) + s1 * (t0 * oldValue[index(i1i, j0i, N)] + t1 * oldValue[index(i1i, j1i, N)]);
        }
    }
...
void project(int N, std::vector<double> &vX, std::vector<double> &vY, std::vector<double> &p, std::vector<double> &div) {
    for (uint32_t i = 1; i < N - 1; i++) {
        for (uint32_t j = 1; j < N - 1; j++) {
            div[index(i, j, N)] = -0.5f * (vX[index(i + 1, j, N)] - vX[index(i - 1, j, N)] + vY[index(i, j + 1, N)] - vY[index(i, j - 1, N)]) / N;
            p[index(i, j, N)] = 0;
        }
    }
...
void lin_solve(int N, Axis mode, std::vector<double> &nextValue, std::vector<double> &value, double diffusionRate) {
    ...
    for (int k = 0; k < ITERATIONS; k++) {
        for (int i = 1; i < N - 1; i++) {
            for (int j = 1; j < N - 1; j++) {
                nextValue[IX(i, j)] =
                        (value[IX(i, j)] + diffusionRate * (nextValue[IX(i + 1, j)] + nextValue[IX(i - 1, j)] + nextValue[IX(i, j + 1)] + nextValue[IX(i, j - 1)])) * cRecip;
            }
        }
